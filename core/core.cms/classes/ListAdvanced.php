...
class ListAdvanced extends ListSimple  
{
  ...
  var $template = 'list_advanced.html:main';
...
//    $tpl->set( '_add_new_href', $this->_add_new_href );
	
//      $tpl->( '_add_new_href',  $this->rh->path_rel."do/".$this->config->module_name."?".$this->rh->state->StatePlus(0, array('_new' => 1)) );
      $tpl->set( '_add_new_href', $this->_href_template.$this->rh->state->StatePlus(0, array('_new' => 1)) );
      $tpl->set( '_add_new_title', $this->config->add_new_title ? $this->config->add_new_title : 'создать новый элемент' );
