...
class DBModel extends Model implements IteratorAggregate, ArrayAccess, Countable, DataContainer
{
	...
	/**
	 * Key of files config
	 * ex. news (supposed, that files config in cms/modules/News/files.yml)
	 * ex. news/items (supposed, that files config in cms/modules/News/conf/items/files.yml)
	 *
	 * @var string
	 */
	protected $files = '';
...
//	}
		...
		if (is_array($config))
		{
			foreach ($config AS $key => $conf)
			{
				...
				if (is_array($conf['children']))
				{
					...
					{
						$this->addField($key.'_'.$subKey, array(
							'type' => 'file',
							'conf' => $configKey.':'.$key.'/'.$subKey
						));
					}
				}
	...
	protected function mapFile($fieldName, &$data)
	{
		...
		$fieldinfo = &$this->foreignFields[$fieldName];
		$data[$fieldName] = FileManager::getFile($fieldinfo['conf'], $data['id']);
	}
