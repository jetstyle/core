   ...
   function _ChooseEventHandler( $event, $handler, $default_method )
   {
      if (isset($this->config[$handler]))
      {
        foreach($this->config[$handler] as $k=>$v)
        {
          ...
          elseif ( is_callable( array($this->config[$handler][$k],$default_method) ) )
          {
...
//     	$this->rh->debug->Trace_R( $dump_hash );
   ...
   function HandleEvent( $event = FORM_EVENT_AUTO ) 
   {
     ...
     if (!$this->processed)
      Controller::Redirect( $this->rh->ri->Href($this->rh->ri->url) );
     else
     {
   ...
   function DbInsert()
   {
      if (!$this->config["db_table"])
     {
      ...
      foreach($values as $k=>$v)
        $values[$k] = Locator::get("db")->Quote($values[$k]);

      $sql = "insert into ".$this->config["db_table"];
      if (sizeof($fields) > 0) 
        $sql.=" (".implode(",",$fields).") VALUES (".implode(",",$values).")";
        
      $this->data_id = Locator::get("db")->Insert($sql);
