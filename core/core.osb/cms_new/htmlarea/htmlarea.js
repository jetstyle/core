  ...
  function initIframe() {
    ...
    if (!editor.config.fullPage) {
      doc.open();
			...
			html += "<script language='javascript' src='" + editor.config.baseURLrel + "js/default.js'></script>\n";
			html += "</head>\n";
			html += "<body class='" + editor.config.bodyClass + "'>\n";
...
//Базовый чистильщик заменён на свой.
//word_cleaner.js - сливки мыслительных процессов Кукуца и Кусо,
//а также результат прогрессивного плиагиата из других источников
//!! та кже здесь подключаются манульаные фильтры
HTMLArea.prototype._wordClean = function() {
  var D = this.getInnerHTML();
  if( word_cleaner.checkWord(D) ){
    this.setHTML( word_cleaner.makeCleanHTML(D) );
    this.updateToolbar();
  }
  //мануальные фильтры
  this.config.ManualFilter(this);
};
  ...
  this._popupDialog("insert_table.html", function(param) {
    ...
    for (var field in param) {
      ...
      switch (field) {
          ...
          case "f_class" : table.className = value; break;
      }
    }
    ...
    //мануальные фильтры
    editor.config.ManualFilter(editor);
    return true;
  }, null);
};
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
  var editor = this;  // for nested functions
// Patch to stop Mozilla using span for bold etc bsutton 
  if (HTMLArea.is_gecko) { 
