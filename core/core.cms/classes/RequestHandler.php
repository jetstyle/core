<?php
require_once (JS_CORE_SITE_DIR . "classes/ConfigProcessor.php");
...
//		$this->DIRS = array_reverse($this->DIRS);
		...
		include($this->FindScript("misc", "startup"));
...
//		}
	...
	function ProceedRequest() {
		...
		if ($this->page[0] == "_")
		{
			$this->EndError("Handler <b>" . $this->page . "</b> is not permitted.");
		}
		$this->missed_OK = true;
		...
		if (!$fname)
			$this->EndError("Cannot read default page <b>" . $this->default_page . "</b>");
		...
		$rh = & $this;
		include ($this->FindScript("misc", "page"));
	...
	function UseClass($name, $level = 0, $direction = 1)
	{
		...
		{
			$fname = $this->FindScript_("classes", $name, $level, $direction);
			require_once ($fname);
	...
	function & UseModule($name, $new_instance = false, $level = 0, $direction = 1) {
		...
		//ищем файл
		$fname = $this->FindScript_("modules", $name, $level, $direction);
		require_once ($fname);
	...
	function getUrl() {    	return 'http://qs/cms/do/Content?id=1';
	}
