...
/*
  ...
  * &Parse( $tpl_root, $store_to, $append ) -- отпарсить по коллекции шаблонок
		- $tpl_root    -- корнева€ шаблонка списка, задаЄм как "file.html:List"
		- $store_to    -- если установлено, то результат также сохран€етс€ в переменную домена с таким именем
								если === true, то записывает результат пр€мо туда, где был задан шаблон
		- $append      -- если непустое $store_to, то результат не стирает значение переменной, а дописываетс€ в конец
  ...
  * $this->implode -- в таком случае список собираетс€ как *_Item  *_Separator  *_Item  *_Separator  *_Item

  * $tpl_root -- корневой шаблон списка
  * $tpl_empty - шаблон пустого списка
  * $tpl_item -- шаблон элемента списка
  * $tpl_separator -- шаблон разделител€ между элементами

  * $list_store_to -- точка входа в корневом шаблоне списка, куда кладЄтс€ собственно список

  // вспомогательные шаблонные переменные
  дл€ каждого р€да:
	 _Num -- номер текущей позиции, начинаетс€ с 1
	 _Even -- признак чЄтности, = $this->loop_index%2
  дл€ корневого шаблона:
...
class ListObject {
	...
	function _do($action){
		...
		//FIXME: ¬ принципе, неправильный аргумент. Raise error?
		return NULL;
	}
