...
class Translit
{
	 ...
	 *
	 * @param string $string
	 * @param int $allow_slashes
	 * @param int $length (0 = unlimited)
	 * @return string
	 */
	//
	function supertag($string, $allow_slashes = TR_NO_SLASHES, $length = 0)
	{
		$slash = "";
		if ($allow_slashes) $slash = "\/";
		...
		{
			$string = trim(substr($string, 0, strrpos(substr($string, 0, $length), '_')));
		}
