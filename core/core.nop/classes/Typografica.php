...
class Typografica
{
	...
	var $glueleft = array( "рис\.", "табл\.", "см\.", "им\.", "ул\.", "пер\.", "кв\.", "офис", "оф\." );
	var $glueright = array( "руб\.", "коп\.", "у\.е\.", "мин\." );

	var $settings = array ( 
		"inches" => 1, // преобразовывать дюймы в &quot;
	...
	function replaceSpecials( $data )
	{
		...
		if ($this->settings["laquo"])
		{
			...
			$data = $this->pregReplace( "/(^|\s|\201|\200|>|\()\"((\201|\200)*[0-9ёЁA-Za-zА-Яа-я\-:\/\.])/i", "\\1&laquo;\\2", $data );
			...
			{
				$_data = $data;
				$data = $this->pregReplace( "/(\&laquo\;([^\"]*)[ёЁA-Za-zА-Яа-я0-9\.\-:\/](\201|\200)*)\"/si", "\\1&raquo;", $data );
				// nb: wacko only regexps follows:
				$data = $this->pregReplace( "/(\&laquo\;([^\"]*)[ёЁA-Za-zА-Яа-я0-9\.\-:\/](\201|\200)*\?(\201|\200)*)\"/si", "\\1&raquo;", $data );
				$data = $this->pregReplace( "/(\&laquo\;([^\"]*)[ёЁA-Za-zА-Яа-я0-9\.\-:\/](\201|\200|\/|\!)*)\"/si", "\\1&raquo;", $data );
			}
		}
		...
		$data = preg_replace( "/(\s|;)\-(\s)/i", "\\1&#150;\\2", $data );
		// 3a. тире длинное
