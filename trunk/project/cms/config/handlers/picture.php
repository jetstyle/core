...
//  $pict = strtr(strtolower($pict),$to_cut);
  foreach($to_cut as $tc=>$v)
	...
	$tpl->assign(array(
		"SRC"=> (@file_exists("files/".$pict))? $rh->back_end->path_rel."pict.php?img=files/".$pict : ((@file_exists("files/catalog_new/".$pict))? $rh->back_end->path_rel."pict.php?img=files/catalog_new/".$pict : "images/0.gif"),
		"TITLE"=>$title,
	));
