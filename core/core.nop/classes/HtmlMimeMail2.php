...
* Last Modified..: $Date: 2004/01/07 23:01:17 $
...
*                          - Default is ISO-8859-1
*/
  function buildMessage($params = array(), $type = 'mail')
  {
    if (!defined('CRLF')) {
    ...
    if (isset($message)) {
      ...
      $this->headers  = array_merge((array)$this->headers, (array)$output['headers']);
      ...
      if (!$this->headers['Message-ID']) $this->headers['Message-ID'] = $message_id;
...
/*    preg_match_all('/(\w*[\x3D\x3F\x20\x80-\xFF]+\w*)/', $input, $matches);
    foreach ($matches[1] as $value) {
      $replacement = preg_replace('/([\x3D\x3F\x20\x80-\xFF])/e', '"=" . strtoupper(dechex(ord("\1")))', $value);
    ...
    foreach ($matches[1] as $value) {
        ...
        $replacement = preg_replace('/([\x20])/e', '_', $replacement); // anti-OutLook
        $input = str_replace($value, '=?' . $charset . '?Q?' . $replacement . '?=', $input);
    }
    ...
    switch ($type) {
      case 'mail':
        ...
        foreach ($this->headers as $name => $value) {
          $headers[] = $name . ': ' . (in_array(strtolower($name), array("subject", "from", "to")) ?
           $this->_encodeHeader($value, $this->build_params['head_charset']) : $value);
        }
