	...
	function Handle()
    {
		...
		switch($action){
			...
			default:
                ...
                $tpl->set('root_id',  "'".(string)$this->getRootId()."'");

				$tpl->set( '_url_connect', $_href.'mode='.$_config_name.'&action=update&_show_trash='.$show_trash.'&' );
                
				$tpl->set( '_url_xml', $_href.'mode='.$_config_name.'&action=xml&_show_trash='.$show_trash.'&'.$this->id_get_var.'='.$this->id.'&' );

				$tpl->set( '_behavior', $this->tree_behavior );
				$tpl->set( '_cur_id', $this->id );
				$tpl->set( '_level_limit', $this->config->level_limit  ? $this->config->level_limit : 100 );
