...
class KCAPTCHA{
	...
	function KCAPTCHA($length=null,$rewriteWidth=0,$rewriteHeight=0){
		...
		if ($rewriteHeight) $height = $rewriteHeight;
		$this->length = $length;
		$fonts=array();
		...
		do{
			...
			*/
			$this->keystring = $this->nopKeystring();
    ...
    function nopKeystring()
    {
        $length = $this->length; // Must be a multiple of 2 !! So 14 will work, 15 won't, 16 will, 17 won't and so on

        // Password generation
        //abcdeghkmnpqsuvxyz
        //$allowed_symbols = "bcdghkmnpqsvxyz"; #alphabet without similar symbols (o=0, 1=l, i=j, t=f)

        $conso=array("b","c","d","g","h","k","m","n","p","q","s","v","x","y","z");
        $vocal=array("a","e","u");
        $password="";
