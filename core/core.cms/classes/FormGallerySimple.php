...
class FormGallerySimple
{
	...
	function Handle()
	{
		...
		$tpl->set('_rubric_id', intval(RequestInfo::get($this->config->rubric_var ? $this->config->rubric_var : 'topic_id')));
...
//											'title' => iconv('cp1251', 'UTF-8', $r['title']),
	...
	{
		$id = intval(RequestInfo::get('picid'));
		$rubric_id = intval(RequestInfo::get($this->prefix . 'rubric'));
