...
*/
window.parent.AddTab( 'Nop', FCKLang.DlgImgInfoTab ) ;
...
function OkNop()
{
	...
	param['f_url'] = arr[0];
	param['f_url_big'] = arr[1].replace( 'pict.php?', 'picture?' ).replace('img=files/','pict=');
	param['f_url_big_plain'] = arr[1];
	param['f_id'] = arr[3];
	param['f_width'] = arr[4];
	param['f_height'] = arr[5];
	param['f_big_width'] = arr[6];
	param['f_big_height'] = arr[7];
	//  
    var fields = ["f_alt", "f_align"];
    for (var i in fields) 
    {
       var id = fields[i];
  ...
  //alert(Dump(param));
    html = insertPict(param, "{{tpl_pict|pack_spaces}}", "{{tpl_preview|pack_spaces}}");
    //alert(html);
	FCK.InsertHtml(html);
