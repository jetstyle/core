...
class ListComplete extends ListAdvanced  {
  ...
  function Handle(){
    ...
    $tpl->Assign( '_title', $this->item[ $this->SELECT_FIELDS[1] ] );
    $tpl->Assign( '_save_string', $this->item ? 'сохранить' : 'добавить' );
    $tpl->Assign( 'prefix', $this->prefix );
    $tpl->Assign( 'POST_STATE', $this->state->State(1) );
    if( $this->id )
      $tpl->Parse( $this->template_form.':delete', '__delete' );
    $tpl->Assign( '__form_name', $this->prefix.'_list_form' );
  ...
  function UpdateForm(){
    ...
    if( $this->rh->GetVar( $this->prefix.'update')  ){
      $this->UPDATE_FIELDS = array( $this->SELECT_FIELDS[1] );
      if( $this->id ){
