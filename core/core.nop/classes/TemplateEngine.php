...
class TemplateEngine extends ConfigProcessor
{
	...
	function Parse( $tpl_name, $store_to="", $append=0, $dummy="" ) // -- парсинг шаблона и возвращение результата.
	{
		...
		$tname=preg_replace("/[^\w\x7F-\xFF\s]/", $this->rh->tpl_template_sepfix, str_replace($this->rh->project_dir, '', $file_source));
	...
	//zharik
	function Action( $action_name, &$params, $level=0, $direction=1 ) // -- вызов действия
	{
