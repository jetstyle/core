...
class FormSimple extends DBDataEdit  {
  ...
  var $_template_item = 'form.html'; //basename шаблона формы, если брать его из конфига
  ...
  function FormSimple( &$config )
  {
    ...
    if ($this->config->id_field) $this->id_field = $this->config->id_field;

    //DBDataEdit
//    $config->Read("form");
    if ($config->state_field) $config->SELECT_FIELDS[] = $config->state_field; else $config->SELECT_FIELDS[] = '_state';
    ...
    if( $config->template_item ) $this->template_item = $config->template_item;
    if( !$this->template_item ){
//      $this->template_item = $this->rh->DIRS[$this->config->handlers_type][CURRENT_LEVEL].$this->config->module_name."/form.html";
...
//      $redirect = $this->rh->url.'do/'.$this->config->module_name.'?'.$this->state->State();
    ...
    if( !$item->$id_field && isset($this->config->new) )
      $tpl->AssignRef( '*', $this->config->new );
    else
      $tpl->AssignRef( '*', $item );

    $tpl->Assign( 'prefix', $this->prefix );
    $tpl->Assign( 'POST_STATE', $this->state->State(1) );
    $tpl->Assign( '__form_name', $this->prefix.'_simple_form' );
    $tpl->Assign( '__delete_title', $item->_state!=2 ? 'удалить в корзину' : 'удалить окончательно'  );
...
*/
    if( is_array($this->config->RENDER) ){
      ...
      for($i=0;$i<$N;$i++){
        ...
        switch( $row[1] ){
          case 'checkbox':
            $tpl->Assign( 'checkbox_'.$row[0], $this->item[$row[0]] ? "checked=\"checked\"" : '' );
          break;
          case 'select':
            $_str = '';
            $A =& $row[2];
...
//              $_str .= "<option value='".$_id."' ".( $this->item[$row[0]]==$_id  || (!$this->item["id"] && $_id==$row[3]) ? "selected=\"selected\"" : '' ).">".$_val;
              $_str .= "<option value='".$_id."' ".(($this->item[$this->id_field] && $this->item[$row[0]]==$_id) || (!$this->item[$this->id_field] && $_id==$row[3]) ? "selected=\"selected\"" : '' ).">".$_val;
            }
            $tpl->Assign( 'options_'.$row[0], $_str );
          break;
          case 'radiobutton':
