   ...
   function _GetSize( $file_name )
   {
    ...
    function _UploadFile($data_id)
    {
        ...
        if ($_FILES['_'.$this->field->name][tmp_name])
        {
            ...
            if ($this->field->config["add_to_rubric"])
            {
    ...
    protected function getFilesRubric($file = null) {
        ...
        if ( !array_key_exists($rubricTypeId, $this->filesRubrics) ) {
            ...
            $rubric->loadOne('{type_id} = '.DBModel::quote($rubricTypeId).' AND {module} = '.DBModel::quote($this->field->config["config_key_module"]));

            if (!$rubric['id']) {

                $data = array(
                    'module' => $this->field->config["config_key_module"],
                    'title' => ModuleConstructor::factory($this->field->config["config_key_module"])->getTitle(),
